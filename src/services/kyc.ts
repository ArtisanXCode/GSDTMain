...
export const getDatabaseUserKYCStatus = async (userAddress: string): Promise<KYCStats> => {
...
export const getUserKYCStatus = async (userAddress: string): Promise<{ status: KYCStatus; request?: KYCRequest } | null> => {
  try {
    ...
    /*
    if(dataRes.length > 0) {
      return { status: dataRes[0].status };
    } else {
...
export const submitKYCRequest = async (data: KYCSubmissionData): Promise<void> => {
  try {
    ...
    if(dataRes.length > 0) {
      ...
      const appIdStatus = await getSumsubApplicantStatus(data.user_address, data.sumsub_applicant_id);
